...
Make a new folder called "session3_output" or something similar using `mkdir` Download your four genetic diversity outputs using `scp` (mac) or `pscp` (windows), in exactly the same way as you learned this morning. Once you have done this, open R studio on your laptop and use `read.csv()` and `read.table()` as appropriate to read in your newly created and downloaded data files to four suitably-named objects.

<div class="fold s">
```{r}
rm(list=ls()) #Clear your workspace

het <- read.table("~/Documents/Teaching/PopGenBerlinLGS/session3_output/TriboliumSSGenomes.het",header = T)
pi_MB <- read.table("~/Documents/Teaching/PopGenBerlinLGS/session3_output/TriboliumSSGenomesMB.windowed.pi", header = T)
pi_FA <- read.table("~/Documents/Teaching/PopGenBerlinLGS/session3_output/TriboliumSSGenomesFA.windowed.pi", header = T)
ld <- read.csv("~/Documents/Teaching/PopGenBerlinLGS/session3_output/TriboliumSSGenomes.windowed.ld.csv") #Or whatever you have called it
