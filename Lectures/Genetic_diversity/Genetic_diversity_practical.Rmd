...
Hopefully you figured out that individuals 2014_SOL_MG4955, 2014_SOL_MG4956 and 2014_SOL_MG4957 are from the MB population, while the remaining individuals are from the FA population. Once you have run the above code, sse the command line to take a look at the format of the output files vcftools has returned. Does it makes sense?
...
# Can sort this by first adding 500 to the distance values

MB$dist <- round(((MB$POS2 - MB$POS1)+500),-3)
FA$dist <- round(((FA$POS2 - FA$POS1)+500),-3)
...
Finally, we need to combine the combine windowed data from the two populations, and add a population ID column. Once you have done this, write your output to a `.csv` file.
...
That's our analyses done! Now you can log out of the server, and use SCP to download the genetic results that you have just created. To recap, we are interested in individual heterozygosity (one file, one value per individual), windowed nucleotide diversity (two files - one per population), and windowed LD (one file). Download your four genetic diversity outputs using SCP. Once you have done this, open R studio on your laptop and use `read.csv()` and `read.table()` as appropriate to read in your newly created and downloaded data files to four suitably-named objects.

<div class="fold s">
```{r}
rm(list=ls()) #Clear your workspace

het <- read.table("../Data/TriboliumSSGenomes.het",header = T)
pi_MB <- read.table("../Data/TriboliumSSGenomesMB.windowed.pi", header = T)
pi_FA <- read.table("../Data/TriboliumSSGenomesFA.windowed.pi", header = T)
ld <- read.csv("../Data/TriboliumSSGenomes.windowed.ld.csv") #Or whatever you have called it
...
Fig1A <- mutate(het,Population = c(rep("MB",3),rep("FA",3)),Heterozygosity = 1-(O.HOM./N_SITES)) %>%
  ...
  xlab("")+
  annotate("text",y = 0.24,x = 0.5,label = "A")
...
Fig1B <- rbind(pi_MB,pi_FA) %>%
  ...
  xlab("")+
  ylab(expression(pi))+
  annotate("text",y = 0.013,x = 0.5,label = "B")
...
Fig1C <-  mutate(ld,Population = pop,distkb = dist/1000) %>%   
  ...
  xlab("Distance (kb)")+
  ylab(expression("R"^2))+
  annotate("text",y = 0.92,x = 0,label = "C")
...
Now we can use ggplot2 to plot heterozygosity against population. We will start off by using the function `geom_point`.
...
Now we need to change the background colout. I did this using `theme_bw()`:
...
Now we are ready to make a plot. We will start with the boxplot. Remember that we can use a lot of the same arguments from our previous plot.
...
We can add a notch to the boxplot easily:
...
To add the points behind the boxplot we will use the function `geom_jitter`:
...
There are two problems here. First, the points are getting in the was of the boxplots. Second, they have been coloured by population whereas we would like them to be grey. Any idea how we fix these things?

<div class="fold s">
```{r}
B <- ggplot(pi_both,aes(x = Population, y = PI, col = Population))+
      geom_jitter(col = "grey")+ #Add this before the geom_boxplot() to put the points behind the boxes
      geom_boxplot(notch = T, alpha = 0)+ #alpha makes the background seethrough
      theme_bw()+
      theme(legend.position = "none")+
      xlab("")
...
Done! Now we just need to make the LD plot and stick them all together. Lety's take another look at the LD data:
```{r}
head(ld)
```
...
Notice that distance is in bp, whereas we would like it in kb. We can also add the lines very easily:
...
Ok, we have done our plots! We now just need to make a single, mutipanel plot. Again, there are lots of ways we can do thism and this is definitely something you should explore. For now we will use the `multiplot()` function from the `Rmisc` library. We can just list the plots without specifying anything to do with the layout:
...
##If you've finished
- Use multiplot to crease a multipanelled plot with i) the LD plot on the top, and ii) the heterozygosity plot stretched along the left-hand side, with the pi and LD plots both on the right hand side.
