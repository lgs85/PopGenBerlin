...
Now use `ls -l`, `head` and/or `cat` as appropriate to explore the output files from each of these analyses. What have the analyses done? Have you noticed any problems with the way the analyses have been performed?
...
Now we need to change the background colour. I did this using `theme_bw()`:
...
Now we are ready to make a plot. We will start with the box plot. Remember that we can use a lot of the same arguments from our previous plot.
...
We can add a notch to the box plot easily:
...
There are two problems here. First, the points are getting in the was of the box plots. Second, they have been coloured by population whereas we would like them to be grey. Any idea how we fix these things?
...
Done! Now we just need to make the LD plot and stick them all together. Let's take another look at the LD data:
```{r}
head(ld)
```
...
Ok, we have done our plots! We now just need to make a single, muti-panel plot. Again, there are lots of ways we can do this and this is definitely something you should explore. For now we will use the `multiplot()` function from the `Rmisc` library. We can just list the plots without specifying anything to do with the layout:
