...
## The data
We will be using a large SNP dataset from nine European great tit populations to explore continental-scale genomic structure. The data are stored as binary plink files in the "population_structure" folder. **Make sure you have copied the population_structure folder from Dati to your home directory.**
...
Now use what you have learned about plink and VCFtools, along with the plink website, to filter the dataset (HINT: filtering in plink and VCFtools is a very similar process!). You should filter based on Hardy-Weinberg (only include loci  with P > 0.01) and minor allele frequencies (only include loci with MAF > 0.05). Give your output the name "GreatTitsFiltered".
...
This is almost there, but because only one of our three admixture plots (the one due to go in panel 5) has x axis labels, we need to ascribe more space to this panel. Can you figure out a way to do this, and produce the layout shown below?
