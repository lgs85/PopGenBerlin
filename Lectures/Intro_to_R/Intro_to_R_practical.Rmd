...
This gives us `r nrow(dd)` `TRUE` or `FALSE` values, and tells us whether each individual in our dataset is a homozygote for SNP1 or not. Now all we need to a vector of values, indicating whether each individual is a homozygote or a heterozygote. We can do this using the `ifelse()` function. Check out what this function does using `?ifelse`. Can you generate a column saying whether each individual in your dataset is a homozygote or heterozygote for the first SNP? You should end up with something like this:
...
- Following this, can you make a plot like the below?
...
- Simulate your own SNP dataset by developing your own code.

- Make the above bar plot, and a stacked version of the above plot, using ggplot2