...
Now open R studio. Either by typing in the console, or using the menu, set your working directory to the "intro_to_r" folder. Now, in your script, type in the commands to read in the "Mainland_Island_SNP_data.csv" file, and assign the dataframe to a suitable object name. You can do this using something like the following code:

```{r}
setwd("~/Documents/Teaching/PopGenBerlin/Lectures/intro_to_r")
dd <- read.csv("Mainland_Island_SNP_data.csv")
```

You will need to change the command above, depending on where the "intro_to_r" folder is stored on your computer. Also note how the command for changing the working directory in R differs to the corresponding command in bash and python.

Once you are happy, run the commands. Make sure that you understand what these commands are doing. Click on the "environment" tab in the top-right panel of R studio. The object `dd` should now be listed. If so, the data has been successfully loaded into R. 
