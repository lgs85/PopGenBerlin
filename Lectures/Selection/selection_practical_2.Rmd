...
## The data
We will be using the great tits data set from the population structure practical.

Because in this session we are looking for loci under selection, we do not want to filter based on Hardy-Weinberg, or to prune based on LD, as this would remove loads of interesting information. However, we still want to exclude unrelated individuals. You can create such a file directly from the population_structure folder using a single line of plink code.
...
```{r}
library(data.table)
pop_fst <- fread("~/Documents/Teaching/PopGenBerlinLGS/selection/GreatTitsUnrel.fst")
...
ad_fst <- fread("~/Documents/Teaching/PopGenBerlinLGS/selection/GreatTitsK4.fst")
...
<div class="fold s">
```{r}
ev <- read.table("~/Documents/Teaching/PopGenBerlinLGS/selection/null.eigenvec")
head(ev)
```
</div>
...
eg1 <- fread("~/Documents/Teaching/PopGenBerlinLGS/selection/null.1.egwas",data.table = F) #some issues with this text file - use read.table as it's simpler

eg1 <- read.table("~/Documents/Teaching/PopGenBerlinLGS/selection/null.1.egwas", header = T,stringsAsFactors = F) #Slower but works
str(eg1)
...
Finally, we will use the `manhattan()` function in the qqman to produce a Manhattan plot of the corrected EigenGWAS p-values. Try to reproduce the plot below, using help that you can find online. Then see if your code matches mine.
...
How do you interpret these plots? Do we have to worry about fasle positives in our data? Which points are "significant"? Is the correction working?
...
- Manhattan plots of sliding window FST (see the "sliding_window.r" script - can you get this working?)
...
- Commbining your selection and population structure plots into a single figure.
